...
var lignetab="";
for (var i = 0; i < tab.length; i++) {//scan toute la ligne
	lignetab="<tr>";//document.getElementById('Classement').innerHTML+="<tr>" dom (getElement.......) interprète et ferme automatiquement idem demande ajout <td> génère <tr><td></td></tr>
	for (var x = 0; x < tab[i].length; x++) {//scan toutes les cases d'une ligne
		//console.log(tab[i][x]);
	lignetab+="<td>"+tab[i][x]+"</td>"// ajoute à ligne autant de case que le tableau initial en contenait
	//document.getElementById('Classement').innerHTML+="<td>"+tab[i][x]+"</td>";
	}
lignetab+="</tr>"
document.getElementById('Classement').innerHTML+=lignetab;
//document.getElementById('Classement').innerHTML+="</tr>"
}
